...
class Twitter:
    def __init__(self, _auth_token=None, proxies=None):
        self.http = httpx.AsyncClient(verify=False, timeout=50, proxies=proxies)
...
class galxe:
    def __init__(self, privateKey, _auth_token=None, W=None, nstproxy_Channel=None, nstproxy_Password=None, kopeechka_token=None):
        try:
            ...
            self.gcaptcha = gcaptcha(proxies)
            self.twitter = Twitter(_auth_token=_auth_token, proxies=proxies)
            self.http = httpx.AsyncClient(verify=False, timeout=50, proxies=proxies)
            self.http.cookies.update({'auth_token': _auth_token})
            self.W = W
            self.mail_name = None
...
if __name__ == '__main__':
    ...
    _nstproxy = f"http://{_nstproxy_Channel}-residential-country_ANY-r_5m-s_BsqLCLkiVu:{_nstproxy_Password}@gw-us.nstproxy.com:24125"
